var Review = function (fname, lname, location, title, desc, rate) {
    this.fname = fname;
    this.lname = lname;
...
Review.prototype.getFullName = function () {
    return this.fname + " " + this.lname;
};
Review.prototype.getStars = function () {
    var template = '<i class="{{cls}}"></i>', empty = 'md-star-outline', full = 'md-star', list = '';
    for (var i = 0; i < 5; i++) {
        var cls = (i < this.rate) ? full : empty;
...
var Reviews = {
    list: [],
    init: function () {
        Reviews.fetch(function (res) {
            Reviews.list = Reviews.parse(res);
            Reviews.build();
            $('.cards').Slider();
        });
    },
    fetch: function (callback) {
        $.ajax({
            url: 'public/scripts/reviews.json',
    ...
    build: function () {
        var list = '',
            template = '<div class="card"><span class="star {{starClass}}">{{stars}}</span><span class="avatar"><i class="md-person"></i></span><a href="#" class="name">{{name}}</a><span class="title">{{title}}</span><span class="desc">{{desc}}</span></div>';

        Reviews.list.forEach(function (review) {
            list += template.replace("{{desc}}", review.desc)
                .replace("{{title}}", review.title)
                .replace("{{name}}", review.getFullName())
                .replace("{{starClass}}", Utils.translateNumToChar(review.rate))
                .replace("{{stars}}", review.getStars());
        });
        $('#place_card').html(list);
    }
};
